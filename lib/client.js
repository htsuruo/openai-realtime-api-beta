...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  /**
   * Registers a concurrent agent that will automatically process new user messages
   ...
   * @returns {Function} Cleanup function to remove the agent
   */
  registerConcurrentAgent(agent_id, prompt_instructions, tool_definition, handler) {
    if (!this.isConnected()) {
      throw new Error('Client must be connected before registering a concurrent agent');
    }

    // Store agent configuration
    this._concurrentAgents = this._concurrentAgents || {};
    this._concurrentAgents[agent_id] = {
      prompt_instructions,
      tool_definition,
      handler
    };

    // Create event handler for agent responses
    const responseHandler = (realtimeEvent) => {
      const serverEvent = realtimeEvent.event;
      if (serverEvent.type === "response.done" && 
          serverEvent.response.metadata?.agent_id === agent_id) {
        const rawMsg = serverEvent.response?.output?.[0]?.arguments ?? "{}";
        try {
    ...
    // Create event handler for new user messages
    const messageHandler = (realtimeEvent) => {
      const serverEvent = realtimeEvent.event;
      if (serverEvent.type === "conversation.item.created" &&
          serverEvent.item.role === "user") {
        // Add a small delay to ensure conversation is initialized
        setTimeout(() => {
          if (this.isConnected() && this.conversation.getItems().length > 0) {
            this.addConcurrentAgent(
              prompt_instructions,
              agent_id,
              serverEvent.item.id,
              tool_definition,
    ...
    this.on("realtime.event", responseHandler);
    this.on("realtime.event", messageHandler);

    // Return cleanup function
    return () => {
      this.off("realtime.event", responseHandler);
      this.off("realtime.event", messageHandler);
      delete this._concurrentAgents[agent_id];
    };
  }

  /**
   * Adds a concurrent agent for a specific message
   ...
   * @returns {true}
   */
  addConcurrentAgent(prompt_instructions, agent_id, message_id, tool_definition, handler) {
    if (!this.isConnected()) {
      throw new Error('Client must be connected before adding a concurrent agent');
    }
    
    // Add event handler for this agent
    this.on("realtime.event", (realtimeEvent) => {
      const serverEvent = realtimeEvent.event;
