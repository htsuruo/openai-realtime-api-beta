...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    ...
    this.concurrentAgents = new Map();
    this._hasUserMessageHandler = false;
    this._resetConfig();
  ...
  registerConcurrentAgent(agent_id, prompt_instructions, tool_definition, handler) {
    ...
    this.concurrentAgents.set(agent_id, {
      instructions: prompt_instructions,
      tool: tool_definition,
      handler: handler
    });

    // Set up response handler for this agent
    ...
    // Set up automatic triggering for new user messages
    if (!this._hasUserMessageHandler) {
      this._hasUserMessageHandler = true;
      this.on("realtime.event", (realtimeEvent) => {
        const serverEvent = realtimeEvent.event;
        if (serverEvent.type === "conversation.item.created" &&
          ...
          setTimeout(() => {
            if (this.isConnected() && this.conversation.getItems().length > 0) {
              // Trigger all registered agents
              for (const [agentId, config] of this.concurrentAgents) {
   ...
   * @returns {true}
   */
  unregisterConcurrentAgent(agent_id) {
    if (!this.concurrentAgents.has(agent_id)) {
      throw new Error(`No concurrent agent found with ID: ${agent_id}`);
    }
    this.concurrentAgents.delete(agent_id);
    return true;
  }
